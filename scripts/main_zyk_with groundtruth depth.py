...
def init_params_zyk(init_pt_cld, w2c, num_frames, intrinsics, time_idx, first_init=True):
    ...
    num_pts = init_pt_cld.shape[0]
    means3D = init_pt_cld[:, :3]  # [num_gaussians, 3]
    colors = np.tile([0.5, 0.5, 0.5], (num_pts, 1)) 
    # colors = init_pt_cld[:, 3:6]# [num_gaussians, 3]
    unnorm_rots = np.tile([1, 0, 0, 0], (num_pts, 1)) # [num_gaussians, 3]
    logit_opacities = torch.zeros((num_pts, 1), dtype=torch.float, device="cuda")
    ...
    # pts_in_cam = (w2c @ torch.cat((pts4, torch.ones_like(pts4[:, :1])), dim=-1).transpose(0, 1)).transpose(0, 1)
...
def tracking(config: dict, params, variables, tracking_curr_data, time_idx):
    ...
    if time_idx > 0:
        ...
        tracking_curr_data['w2c'] = tracking_curr_data['gt_w2c']
...
def mapping(config: dict, params, variables, curr_data, time_idx, keyframe_list):
    ...
    curr_data['w2c'] = curr_data['gt_w2c']
    color = curr_data['im']
...
def update_pointcloud_with_depth(sparse_pt_cld, depth, intrinsics, w2c):

    sparse_pt_cld = sparse_pt_cld.to(depth.device)

    c2w = torch.inverse(w2c)
    pts4 = torch.cat((sparse_pt_cld, torch.ones(sparse_pt_cld.shape[0], 1).to(sparse_pt_cld.device)), dim=1)
    pts_cam = (c2w @ pts4.T).T[:, :3]
    ...
    u = torch.clamp(u, 0, depth.shape[2] - 1).long()
    v = torch.clamp(v, 0, depth.shape[1] - 1).long()


    depth_values = depth[0, v, u]


    sparse_pt_cld[:, 2] = depth_values
   

    return sparse_pt_cld
...
def mono_slam(config: dict):
    ...
    output_dir = os.path.join(config["workdir"], config["run_name"],timestamp)
    ...
    for time_idx in range(0, num_frames):
        ...
        curr_data = {'cam': cam, 'intrinsics': intrinsics, 
                     ...
                     'w2c': gt_w2c, 'gt_w2c': gt_w2c, 'last2curr': relative_pose}
        if init_pt_cld is not None:
            init_pt_cld = update_pointcloud_with_depth(init_pt_cld, depth, intrinsics, w2c)
            params, variables = update_params_zyk(params, variables, curr_data, init_pt_cld, num_frames, intrinsics, time_idx)
            # params, variables = initialize_params(init_pt_cld, num_frames, mean3_sq_dist, use_simplification=True)       
        if time_idx == 0:
            pbar.update(1)
            continue
...
if __name__ == "__main__":
    ...
    # ZYK
    args.experiment = "configs/c3vd/c3vd_base.py"
    ...
    # Create Results Directory and Copy Config    results_dir = os.path.join(
    timestamp = time.strftime("%Y%m%d-%H%M%S")

    results_dir = os.path.join(experiment.config["workdir"], experiment.config["run_name"],timestamp)
